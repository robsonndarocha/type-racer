...
class Game extends Component {
  ...
  componentDidMount() {
    ...
    socket.on('self.joined', ({ quote, players }) => this.setState({ quote, players }));

    socket.on('player.joined', ({ player }) => this.setState({ players: this.state.players.concat(player) }));

    socket.on('player.left', (id) => this.setState({ players: this.state.players.filter(i => i.id !== id) }));

    socket.on('player.ready', (player) => this.setState({ players: [...this.state.players.filter(i => i.id !== player.id), player] }));

    socket.on('game.ranking', (ranking) => this.setState({ ranking }));

    socket.on('game.start', () => {
      const warnStart = (sec) => {
        setTimeout(() => message.info(`Game starting in ${sec}`, 0.5), (5 - sec) * 1000);
      }
